...
> 一款约 20–30 分钟、给机器学习零基础新生玩的浏览器调查游戏：从“橘猫被识别成面包”的事故录像开始，亲手查证为什么 **训练集 100% 也可能是坏消息**。
...
Playwright E2E 会在真实 Chromium 中完成新版整案：Cold Open → 观察判断 → 读取传感器 → 首次训练与预测 → 未知审计失败 → 两条误判证据 → k=1 过拟合实验 → 备用传感器修复 → 最终判断 → `CASE CLOSED`；另有一条 debug E2E 防止新手门槛破坏工程测试模式。

...
剧情案件也使用版本化浏览器本地检查点：关闭页面或刷新后会先出现 `UNFINISHED CASE SAVED`，明确显示当前阶段、实验数和剩余正式审计额度，再由玩家决定继续或二次确认清档重开。已经锁定但尚未执行的实验预测也会恢复，因此 F5 不能抹掉“先下注、再验证”的实验记录；结案后则可重新打开 `RESOLVED CASE SAVED` 案卷。
...
- **剧情长局也不会被 F5 清空。** Story Case 会恢复 reducer、微阶段、实验预注册、已调查误判与有限审计预算；右上角小型 `RESET` 需要二次确认才真正清档。
...
标题页以剧情案件为主入口；“已熟悉流程？进入无尽调查”会先进入模式说明与训练案件。开发 / 复现时也可以用 query 直接固定正式无尽 seed：

...
项目不再维护一套与正式游戏平行的 Debug UI。最省事的方式是在任意游戏 URL 后加 `qa=1`，右下角会出现 **QA BENCH / OPEN**；也可以按 **`**（Backquote）或 **Ctrl+Shift+K** 打开同一工作台。里面直接提供 CASE 001 各关键阶段、CASE 002、CASE 003、CASE 004、Bureau、Training、四类代表 Duty 的卡片，并可输入任意 Duty seed。所有跳转继续先备份正式 `aia.*` 存档，测试结束可一键恢复。

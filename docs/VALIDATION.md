...
- Vite production build：通过；当前冻结 runtime 构建为 `assets/index-Bk8eCIoo.js` + `assets/index-lQM9_nnH.css`（生产发布后再记录远端哈希）。
...
- 竞争假设状态：`H-FIELDS / H-MODEL` 各自只读取玩家 run history；未测试为 OPEN，material controlled change 为 SUPPORTED，真正测试过但变化不足 12pt 为 WEAKENED。两个轴可以同时 SUPPORTED，此时 UI 明确提示单一主因解释不足。causal pre-registration 对 fields-only / model-only 正式审计是必填项：未选择“应该明显变化 / 应该基本不变”时审计按钮保持 disabled，运行时 `audit()` 也会拒绝绕过 UI 的调用；弱变化只有在审计前已留下该因果预期时，才可通过 `preRegisteredNullResult()` 作为正式 falsification gate，禁止看完结果后再把普通 null result 包装成反证。实验日志现在同时封存因果预期与实际 `material/null` 结果，并标记预测命中或失误，因此两种预注册选择不再只是形式按钮：玩家能直接看到自己的因果预测是否被数据推翻。

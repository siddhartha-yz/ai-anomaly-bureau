...
- Vite production build：通过；当前冻结 runtime 构建为 `assets/index-Bzh3w87K.js` + `assets/index-B__UcwdL.css`（生产发布后再记录远端哈希）。

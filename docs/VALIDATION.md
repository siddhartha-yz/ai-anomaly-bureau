...
- 调查评级：错误上线、推理修正、预测偏差、额外审计和提示会降低评级；S 只保留给干净证据路线。Duty 的方向性 causal pre-registration 由 `causalForecastStats()` 汇总，baseline / repeat / mixed 不计入；来源预判由 `caseLeadForecastStats()` 只统计实际打开且有历史预测的来源。`dutyForecastCalibrationPenalty()` 在既有封顶分之后对每次因果方向 miss 扣 3 分、每次来源预判 miss 扣 2 分，所以错误预注册仍保留为有效证据，但不再与校准良好的推理同分；旧 v6 已打开但没有预测记录的来源不追溯扣分。真实结案 E2E 使用一条 1 hit / 1 miss 的来源路线，检查 `SOURCE FORECAST 1/2`、`评级 - 2` 与最终评分明细，同时继续检查因果预测扣分。
...
- causal-source pre-registration：每份 READY 来源在解封前必须先选择 `预测 SIGNAL / 预测 CLEAR`，否则复核按钮保持 disabled，runtime 也拒绝未预测的直接打开。预测在打开前可改、打开后锁定并显示 HIT/MISS；`caseLeadForecastStats()` 只统计真正打开过的来源。v6 session 以可选 `caseLeadPredictions` 向后兼容保存，专项测试覆盖合法往返、未知值拒绝、旧 v6 无字段继续恢复；Chromium 代表路线同时验证 CLEAR→CLEAR 与 SIGNAL→SIGNAL 两种命中。

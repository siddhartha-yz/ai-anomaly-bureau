...
- `CASE 004+ / SEALED`：只表示后续手工模块尚未编入，不由 Duty 工单代替。
...
所有手工正式案件必须先注册到 `src/bureau/catalog.ts`，并在 `src/story/registry.tsx` 提供对应 runtime。编号、标题、事故摘要、调查目标、分类标签、玩家展示 tags 与前置案件由 catalog 统一提供；runtime 负责组件、checkpoint 摘要、清档与“已结案 checkpoint → Bureau 长期事实”的 reconciliation。Hub 直接遍历 `FORMAL_CASE_CATALOG`，不会为 CASE 001/002/003 各手写一套特殊案件卡。当前目录登记三宗正式案件，并由 `unlockAfter` 形成 `001 → 002 → 003` 的前置链；训练案件仍保存在独立 `TRAINING_CASE_CATALOG`。
...
CASE 002 新增一个指标层原语：玩家先亲眼遇到“总体 93% / 优先病例召回 25%”，再通过真实阈值计算在漏报与误报之间找到同时满足安全约束的方案。CASE 003 再把 CASE 001 的未知审计与 CASE 002 的分类别召回复用起来，只改变观察通道、不改变简单分类规则，比较白天历史与夜班现场，从而把“分布变化”变成可操作的控制变量谜题。

这里借鉴的是组合式关卡结构而不是照搬任何具体题目：`新原语 → 立即使用 → 下一案与旧原语组合`。目标是让 CASE 004+ 能继续向数据泄漏、校准、偏差/公平性、表示与神经网络等复杂系统生长，而不退化成术语问答。

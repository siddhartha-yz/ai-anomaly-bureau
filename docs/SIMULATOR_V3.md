...
下一步仍优先补模拟器本身，而不是内容：更值得验证的是打开后的结构怎样自然重新封装、层级调试如何建立进入/退出语义，以及更一般的 sample primitive。基础 wire probe 已能持续盯住中间信号；后续若继续增强探针，应优先考虑条件暂停/比较，而不是堆更多静态仪表。只有当 Sandbox 自身已经值得摆弄，再讨论关卡系统。

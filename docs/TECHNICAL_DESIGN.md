...
病因名称采用更严格的 progressive disclosure：`diagnosisAvailable` 同时要求至少两个不同配置、存在 fresh material single-variable experiment、已经找到 `accuracy >= .85 && min recall >= .75` 的可靠方案、至少主动复核一份 causal source，并满足 **falsificationReady**。后者只有两种来源：① 已打开 source 的 `result === clear`，客观排除了一个因果故事；② 当前 material support 所在轴的**竞争轴**在审计前已经留下 causal pre-registration，随后实际变化 <12pt，形成可追溯的 `WEAKENED` null result。`competingAxisNullResult()` 明确要求 fields support 配 model null，或 model support 配 fields null；而且这个 competing-axis null 必须锚定在 material support 两个端点之一的非竞争配置上（fields support 时，model null 必须固定在 support 的旧字段或新字段；model support 时，fields null 必须固定在 support 的旧模型或新模型）。因此同一轴上的 material + null，以及第三套无关配置上的 competing-axis null，都不能被拼成完整“支持 + 反证”链。引用诊断证据时也重新按所引用 material 对照的轴检查这一条件，避免引用一组支持证据、却拿另一组无关 null 过 gate。没有预注册的弱变化仍可削弱 UI 中的轴状态，但不会被 `preRegisteredNullResult()` 当作正式 falsification gate。`causalPredictionResult()` 另外把每次已预注册的单变量实验归一成 `expected / observed / hit`，把 material result 再区分为 `improved / degraded`，供实验日志显示“应改善 / 应恶化 / 应基本不变 → 实际明显改善 / 明显恶化 / 基本不变”，让方向判断本身成为可复盘记录。只有支持证据而没有独立反证时，`NEXT OBJECTIVE` 会停在 `CAUSE / FALSIFY` 或证据引用阶段，诊断面板会明确提示去测试另一条干预轴或复核一个 `clear` source，syndrome 选项仍不会开放提交。

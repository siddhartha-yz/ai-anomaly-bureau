...
export function createRuntimeSession(graph: SimulatorGraph): RuntimeSession {
  ...
  return { tick: 0, totalTicks: streamClockLength(graph), values: {} }
}

function evaluateStreamTick(graph: SimulatorGraph, session: RuntimeSession): RuntimeFrame {
  ...
  return { tick, totalTicks: session.totalTicks, result: { steps, values } }
}

export function stepRuntimeSession(graph: SimulatorGraph, session: RuntimeSession) {
  const frame = evaluateStreamTick(graph, session)
  return {
    frame,
    session: { tick: frame.tick, totalTicks: frame.totalTicks, values: frame.result.values } satisfies RuntimeSession,
  }
}

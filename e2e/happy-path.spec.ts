...
import { puzzleSessionKey } from '../src/story/puzzleSession'
...
test('QA Test Bench protects normal saves across Story and Duty jumps, then restores them exactly', async ({ page }) => {
  ...
  terminal = page.getByRole('dialog', { name: '作弊码终端' })
  await terminal.getByLabel('QA 测试工作台').getByRole('button', { name: /CASE 002 · 指标谜题/ }).click()
  await expect(page.getByLabel('CASE 002 调查终端')).toBeVisible()
  await expect(page.getByRole('heading', { name: '92% 为什么仍然不安全？' })).toBeVisible()

  await page.getByRole('button', { name: '打开 QA 测试工作台' }).click()
  terminal = page.getByRole('dialog', { name: '作弊码终端' })
  await terminal.getByLabel('QA 测试工作台').getByRole('button', { name: /CASE 003 · 环境谜题/ }).click()
  await expect(page.getByLabel('CASE 003 调查终端')).toBeVisible()
  await expect(page.getByRole('heading', { name: '同一个模型，为什么天黑就坏？' })).toBeVisible()

  await page.getByRole('button', { name: '打开 QA 测试工作台' }).click()
  terminal = page.getByRole('dialog', { name: '作弊码终端' })
  await terminal.getByLabel('QA 测试工作台').getByRole('button', { name: /CASE 004 · 泄漏谜题/ }).click()
  await expect(page.getByLabel('CASE 004 调查终端')).toBeVisible()
  await expect(page.getByRole('heading', { name: '这个“验证集”真的没见过吗？' })).toBeVisible()
...
test('qa=1 exposes a visible Test Bench launcher while normal player URLs remain clean', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate((key) => window.localStorage.getItem(key), puzzleSessionKey(STORY_CASE_004.id, 20260809))).not.toBeNull()

  await page.getByRole('button', { name: '打开 QA 测试工作台' }).click()
  const resumedTerminal = page.getByRole('dialog', { name: '作弊码终端' })
  await resumedTerminal.getByLabel('任意 DUTY SEED').fill('7421')
  await resumedTerminal.getByRole('button', { name: '打开指定 Duty' }).click()
...
test('Bureau Hub turns solved content into one persistent investigation workspace', async ({ page }) => {
  ...
  await expect(shiftPriority).toContainText('CASE 002 · 被平均数藏起来的人')
  await qaShot(page, '54-bureau-shift-priority')
  await shiftPriority.getByRole('button', { name: '前往案件板' }).click()
  await expect(bureauDepartment(page, /案件板/)).toHaveAttribute('aria-pressed', 'true')
...
test('authored CASE 002 through CASE 005 grow one reusable investigation primitive at a time', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '92% 为什么仍然不安全？' })).toBeVisible()
  await page.getByRole('button', { name: /按类别拆开召回/ }).click()
  await page.getByRole('button', { name: '接入仪表' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('优先病例召回')
  await expect(page.getByLabel('本次调查结果')).toContainText('25%')
  await page.getByRole('button', { name: '封存证据 · 进入下一谜题' }).click()
  await expect(page.getByRole('heading', { name: '把一个数字变成真正的约束谜题' })).toBeVisible()
  await expect.poll(() => page.evaluate((key) => window.localStorage.getItem(key), puzzleSessionKey(STORY_CASE_002.id, seed))).not.toBeNull()

  // A refresh returns to the Hub, where the generic formal-case checkpoint card
  // owns recovery instead of CASE 002 inventing a parallel navigation system.
  await page.reload()
  await expect(hub).toBeVisible()
  await expect(page.getByRole('button', { name: '继续 CASE 002' })).toBeVisible()
  await page.getByRole('button', { name: '继续 CASE 002' }).click()
  const resume = page.getByLabel('已保存剧情案件')
  await expect(resume).toContainText('CHECKS')
  await expect(resume).toContainText('REVISIONS')
  await resume.getByRole('button', { name: '继续上次调查' }).click()
  await expect(page.getByRole('heading', { name: '把一个数字变成真正的约束谜题' })).toBeVisible()

  // Deliberately trust the attractive 93% solution once. The evidence rejects it,
  // then the player can recover without restarting the case.
  await page.getByRole('button', { name: /阈值 0\.80/ }).click()
  await page.getByRole('button', { name: '运行未知审计' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('HYPOTHESIS REJECTED')
  await expect(page.getByLabel('本次调查结果')).toContainText('25%')
  await page.getByRole('button', { name: /阈值 0\.55/ }).click()
  await page.getByRole('button', { name: '运行未知审计' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('约束同时满足')
  await expect(page.getByLabel('本次调查结果')).toContainText('100%')
  await page.getByRole('button', { name: '封存证据 · 进入下一谜题' }).click()

  await page.getByRole('button', { name: /总体准确率可能更高，但系统反而更危险/ }).click()
  ...
  await expect(page.getByText('类别不平衡')).toBeVisible()
  await bureauDepartment(page, /案件板/).click()

  await page.getByRole('button', { name: '接收 CASE 003' }).click()
  await expect(page.getByLabel('CASE 003 调查终端')).toBeVisible()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 2)).toBe(true)
  await page.getByRole('button', { name: /历史 \/ 夜班采集条件/ }).click()
  await page.getByRole('button', { name: '调取证据' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('夜班现场')
  await expect(page.getByLabel('本次调查结果')).toContainText('50%')
  await expect(page.getByLabel('本次调查结果')).toContainText('0%')
  await page.getByRole('button', { name: '封存证据 · 进入下一谜题' }).click()

  // CASE 003 accepts more than one stable sensor: this is a system puzzle, not
  // a multiple-choice question with one arbitrary magic button.
  await page.getByRole('button', { name: /局部纹理/ }).click()
  await page.getByRole('button', { name: '只换观察通道并审计' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('跨环境基本稳定')
  await page.getByRole('button', { name: '封存证据 · 进入下一谜题' }).click()
  await page.getByRole('button', { name: /亮度捷径在夜班发生了分布变化/ }).click()
  await page.getByRole('button', { name: '提交因果解释' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('因果链闭合')
  await page.getByRole('button', { name: '封存证据并结案' }).click()
  await expect(page.getByLabel('CASE 003 已结案')).toContainText('S')
  await expect.poll(() => page.evaluate((key) => window.localStorage.getItem(key), puzzleSessionKey(STORY_CASE_003.id, seed))).not.toBeNull()
  await page.getByRole('button', { name: '归档并返回调查局' }).click()
  ...
  await expect(page.getByLabel('CASE 004 调查终端')).toBeVisible()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 2)).toBe(true)
  const splitLedger = page.getByLabel('这个“验证集”真的没见过吗？ 证据档案')
  await expect(splitLedger).toContainText('SPLIT_LEDGER')
  await expect(splitLedger.getByText('OBJ-09')).toHaveCount(2)
  ...
  // leaks half of the physical objects. Then switch to entity-level isolation.
  await page.getByRole('button', { name: /按拍摄日期切分/ }).click()
  await page.getByRole('button', { name: '重切分并验证' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('HYPOTHESIS REJECTED')
  await expect(page.getByLabel('本次调查结果')).toContainText('50%')
  await page.getByRole('button', { name: /按物品实体分组切分/ }).click()
  await page.getByRole('button', { name: '重切分并验证' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('身份重叠')
  await expect(page.getByLabel('本次调查结果')).toContainText('0%')
  await page.getByRole('button', { name: '封存证据 · 进入下一谜题' }).click()

  await page.getByRole('button', { name: /材料结构分类器/ }).click()
  await page.getByRole('button', { name: '运行干净验证' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('88%')
  await expect(page.getByLabel('本次调查结果')).toContainText('75%')
  await page.getByRole('button', { name: '封存证据 · 进入下一谜题' }).click()
  await page.getByRole('button', { name: /按真实部署中的“新对象单位”隔离训练与验证/ }).click()
  ...
  await expect(page.getByLabel('本次调查结果')).toContainText('9%')
  await page.getByRole('button', { name: '封存证据 · 进入下一谜题' }).click()

  const calibrationSplit = page.getByLabel('保持排序不变，只修正概率刻度 证据档案')
  await expect(calibrationSplit).toContainText('CALIBRATION_SPLIT')
  await expect(calibrationSplit).not.toContainText('FITTED OUTPUT')
  await expect(calibrationSplit).toContainText('70%')
  await page.getByRole('button', { name: /冻结 20% \/ 40% \/ 60% \/ 80%/ }).click()
  await page.getByRole('button', { name: '冻结校准方案' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('HYPOTHESIS REJECTED')
  await expect(page.getByLabel('本次调查结果')).not.toContainText('68%')
  await page.getByRole('button', { name: /冻结 10% \/ 30% \/ 70% \/ 90%/ }).click()
  await page.getByRole('button', { name: '冻结校准方案' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('映射已冻结')
  await expect(page.getByLabel('本次调查结果')).not.toContainText('68%')
  await page.getByRole('button', { name: '封存证据 · 进入下一谜题' }).click()

  const heldOutAudit = page.getByLabel('映射冻结后，最终审计还能拿来做什么？ 证据档案')
  await expect(heldOutAudit).toContainText('SEALED_AUDIT')
  await expect(heldOutAudit).toContainText('68%')
  await expect(heldOutAudit).toContainText('92%')
  await page.getByRole('button', { name: /校准明显改善，但独立审计仍允许存在残余误差/ }).click()
  await page.getByRole('button', { name: '提交审计结论' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('原始 ECE')
  await expect(page.getByLabel('本次调查结果')).toContainText('9%')
  await expect(page.getByLabel('本次调查结果')).toContainText('冻结映射 ECE')
  ...
  await expect(policyCard.getByRole('columnheader', { name: 'POLICY' })).toHaveCount(0)
  await expect(policyCard).not.toContainText('≥65%')
  await page.getByRole('button', { name: /干预 RAW 60% \+ 80% 两档；把政策改成 55%/ }).click()
  await page.getByRole('button', { name: '执行处置策略' }).click()
  await expect(page.getByLabel('本次调查结果')).toContainText('HYPOTHESIS REJECTED')
  await expect(page.getByLabel('本次调查结果')).toContainText('被偷偷改成 55%')
  await page.getByRole('button', { name: /干预 RAW 60% \+ 80% 两档；政策仍写 65%/ }).click()
  await page.getByRole('button', { name: '执行处置策略' }).click()
...
test('Bureau Hub remains operable on a 1280x720 laptop viewport', async ({ page }) => {
  ...
  await expect(priority).toContainText('CASE 002 · 被平均数藏起来的人')
  await priority.getByRole('button', { name: '前往案件板' }).click()
  await expect(departmentNav.getByRole('button', { name: /案件板/ })).toHaveAttribute('aria-pressed', 'true')
...
test('wrong endless diagnosis remains locked across refresh until fresh evidence is cited', async ({ page }) => {
  ...
  // A wrong diagnosis may still be submitted when it is causally compatible
  // with H-FIELDS, but it must first review its own positive source. Seed 6000
  // carries a benign H-COVERAGE signal, so class imbalance is plausible but wrong.
  await page.getByRole('button', { name: '多数类把总体准确率撑高，少数类却一直漏掉' }).click()
  await expect(page.getByText(/还缺少对应来源的正向事实/)).toBeVisible()
  await expect(page.getByRole('button', { name: '提交诊断' })).toBeDisabled()
  await expect(page.getByLabel('当前调查目标')).toContainText('当前病因还缺一条正向来源事实')
  await expect(page.getByRole('button', { name: '定位下一步操作' })).toContainText('定位：因果线索')
  await inspectCausalLead(page, /历史档案构成/)
  await expect(page.getByLabel('当前调查目标')).toContainText('证据包已就绪，形成病因判断')
  await expect(page.getByRole('button', { name: '提交诊断' })).toBeEnabled()
  ...
  await page.reload()
  await expect(page.getByLabel('已恢复本案进度')).toBeInViewport()
  await expect(page.getByText(/刚提交：多数类把总体准确率撑高，少数类却一直漏掉/)).toBeVisible()
  await expect(page.getByText(/报告已暂时锁定/)).toBeVisible()
  await expect(page.getByRole('button', { name: '观察特征没有抓住真正差异' })).toBeDisabled()
  await expect(page.locator('.endless-objective b')).toHaveText('审计额度 3')
...
test('endless supervised mode rewards evidence-led experiments over random clicking', async ({ page }) => {
  ...
  // A wrong but causally compatible diagnosis still needs its own positive source;
  // seed 6000 carries a benign H-COVERAGE signal, making class imbalance plausible but wrong.
  await inspectCausalLead(page, /历史档案构成/)
  await page.getByRole('button', { name: '多数类把总体准确率撑高，少数类却一直漏掉' }).click()
  await page.getByRole('button', { name: '提交诊断' }).click()
  await expect(page.getByText(/刚提交：多数类把总体准确率撑高，少数类却一直漏掉/)).toBeVisible()
  ...
  await expect(closureReport).toContainText('CAUSAL SUPPORT')
  await expect(closureReport).toContainText('受控实验干预证据')
  await expect(closureReport).toContainText(/没有专属正向来源.*material 单变量实验/)
  ...
  await expect(closureReport).toContainText('SOURCE FORECAST')
  await expect(closureReport).toContainText('1/2 命中')
  await expect(closureReport).toContainText('1 次来源预判与实际不符；用于复盘取证直觉，不计调查评级')
  await expect(page.getByText(/实验设计：2 次单变量对照（前 2 次计设计奖励）.*因果预测 1\/2（1 次失误，- 3）.*来源预判 1\/2（只复盘，不计分）/)).toBeVisible()
...
test('endless mode rejects high overall accuracy when a minority class is still missed', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '提交诊断' }).click()
  await expect(page.getByText('CASE RESOLVED')).toBeVisible()
  const imbalanceClosure = page.getByLabel('无尽案件结案报告')
  await expect(imbalanceClosure).toContainText('CAUSAL SUPPORT')
  await expect(imbalanceClosure).toContainText('历史档案构成 · SIGNAL')
  await expect(imbalanceClosure).toContainText(/正常日志 40 条.*故障日志 4 条/)

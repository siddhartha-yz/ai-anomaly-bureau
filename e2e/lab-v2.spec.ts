...
test('V2 default route is a build-run-fix workbench, not an authored option flow', async ({ page }) => {
  ...
  await expect(page.getByLabel('实验工具架')).toContainText('UNKNOWN MODULE')
  await expect(page.getByLabel('实验工具架')).not.toContainText('TEST PROBE')
  await expect(page.getByLabel('测量仪表')).toContainText('RUN REQUIRED')
  await expect(page.getByLabel('特征总线').getByRole('button', { name: '纹理 + 比例' })).toBeDisabled()
...
test('V2 vertical slice grows three reusable primitives through direct manipulation', async ({ page }) => {
  ...
  await expect(page.getByLabel('测量仪表')).toContainText('RUN REQUIRED')
  await expect(page.getByLabel('测量仪表')).not.toContainText('88%')
  await page.getByRole('button', { name: /SHIP \/ RUN FIELD GATE/ }).click()
  await expect(page.getByLabel('测量仪表')).toContainText('88%')
  ...
  await expect(page.getByLabel('实验工具安装槽')).toContainText('TEST PROBE')
  await expect(page.getByLabel('风险阈值')).toBeDisabled()
  ...
  await expect(page.getByLabel('测量仪表')).toContainText('RUN REQUIRED')
  await expect(page.getByLabel('分类阈值控制器')).not.toContainText(/\d\/4 漏诊/)
  await page.getByRole('button', { name: 'RUN TESTS' }).last().click()
  await expect(page.getByLabel('测量仪表')).toContainText('PRIORITY RECALL')
  ...
  await expect(page.getByLabel('观察通道').getByRole('button', { name: '局部纹理' })).toBeDisabled()
  await page.getByRole('button', { name: 'RUN TESTS' }).last().click()
  await expect(page.getByLabel('最近一次运行结果')).toContainText('夜班部署失败')
  await page.getByRole('button', { name: /ENV SWITCH/ }).click()
  await expect(page.getByLabel('观察通道').getByRole('button', { name: '局部纹理' })).toBeEnabled()
  await page.getByLabel('观察通道').getByRole('button', { name: '局部纹理' }).click()
  await expect(page.getByLabel('测量仪表')).toContainText('RUN REQUIRED')
  await expect(page.getByLabel('观察通道')).not.toContainText(/DAY\s+\d+%/)
  await expect(page.getByLabel('观察通道')).not.toContainText(/NIGHT\s+\d+%/)
  await page.getByRole('button', { name: 'RUN TESTS' }).last().click()

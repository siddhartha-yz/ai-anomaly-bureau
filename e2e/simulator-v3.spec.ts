...
test('default route is an empty construction simulator, not a scripted level', async ({ page }) => {
  ...
  await expect(page.getByLabel('构造画布')).toContainText('空白构造台')
...
test('construction board camera pans and zooms without mutating the machine graph', async ({ page }) => {
  ...
  await expect(page.getByLabel('构造画布')).toContainText('视图 75%')

  await viewport.hover({ position: { x: 420, y: 250 } })
  await page.mouse.wheel(0, -420)
  await expect(page.getByLabel('构造画布')).not.toContainText('视图 75%')
...
test('construction edits can be undone and redone without rebuilding the machine', async ({ page }) => {
  ...
  await expect(page.getByLabel('构造画布')).toContainText('3 元件 · 0 连线')
  await undo.click()
  await expect(page.getByLabel('构造画布')).toContainText('2 元件 · 0 连线')
  await redo.click()
  await expect(page.getByLabel('构造画布')).toContainText('3 元件 · 0 连线')
  ...
  await expect(page.getByLabel('构造画布')).toContainText('3 元件 · 1 连线')
  await undo.click()
  await expect(page.getByLabel('构造画布')).toContainText('3 元件 · 0 连线')
  await redo.click()
  await expect(page.getByLabel('构造画布')).toContainText('3 元件 · 1 连线')
...
test('live wiring rejects feedback loops before they enter the graph', async ({ page }) => {
  ...
  await expect(page.getByLabel('构造画布')).toContainText('2 元件 · 1 连线')
...
test('player can construct, wire, run and step-debug a threshold machine', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '添加 GREATER THAN' }).click()
  await page.getByRole('button', { name: '添加 BOOLEAN OUTPUT' }).click()
  await expect(page.getByLabel('构造画布')).toContainText('4 元件 · 0 连线')
  ...
  await expect(page.getByLabel('构造画布')).toContainText('4 元件 · 3 连线')
  ...
  await expect(page.getByLabel('构造画布')).toContainText('5 元件 · 2 连线')
  await page.getByRole('button', { name: '▶ PLAY' }).click()
...
test('number score streams can flow through a threshold primitive into boolean stream logic', async ({ page }) => {
  ...
  await expect(page.getByLabel('构造画布')).toContainText('5 元件 · 4 连线 · 时钟 0/4')
  await page.locator('.sim-wire-layer g').first().locator('.sim-wire-hit').click()
  await page.getByRole('button', { name: '固定信号探针' }).click()
  ...
  await expect(page.getByLabel('信号探针')).toContainText('3 个样本')
  ...
  await expect(page.getByLabel('number_output_1 输出值')).toHaveText('—')
  await page.getByRole('button', { name: '关闭条件暂停' }).click()
  await page.getByRole('button', { name: '▶ PLAY' }).click()
  ...
  await expect(page.getByLabel('信号探针')).toContainText('4 个样本')
...
test('player can compose a generic match ratio from stream primitives', async ({ page }) => {
  ...
  await expect(page.getByLabel('构造画布')).toContainText('7 元件 · 7 连线')
  await expect(page.getByLabel('构造画布')).toContainText('时钟 0/4')
  await page.getByRole('button', { name: '单步' }).click()
  await expect(page.getByLabel('模拟器状态')).toContainText('SAMPLE 1/4 · NODE 1/7 · 布尔流输入 · BOOLEAN STREAM')
  await expect(page.getByLabel('number_output_1 输出值')).toHaveText('—')
  await expect(page.locator('.sim-wire-layer g.hot')).toHaveCount(1)
  for (let index = 0; index < 6; index += 1) await page.getByRole('button', { name: '单步' }).click()
  await expect(page.getByLabel('模拟器状态')).toContainText('SAMPLE 1/4 · NODE 7/7 · 数值输出 · NUMBER OUTPUT · SAMPLE COMPLETE')
  await expect(page.getByLabel('number_output_1 输出值')).toHaveText('1')
  await expect(page.getByLabel('模拟器状态')).toContainText('1 / 4')
  for (let index = 0; index < 7; index += 1) await page.getByRole('button', { name: '单步' }).click()
  await expect(page.getByLabel('number_output_1 输出值')).toHaveText('0.50')
  await expect(page.getByLabel('构造画布')).toContainText('时钟 2/4')
  await page.getByRole('button', { name: '▶ PLAY' }).click()
  await expect(page.getByLabel('number_output_1 输出值')).toHaveText('0.50')
  await expect(page.getByLabel('模拟器状态')).toContainText('PLAY COMPLETE · 4 个样本时钟已执行')
...
test('player can save a working fragment as a reusable blueprint and place another copy', async ({ page }) => {
  ...
  await expect(page.getByLabel('蓝图工具')).toContainText('已选择 2 个单元')
  await page.getByLabel('蓝图名称').fill('MY GATE')
  await page.getByRole('button', { name: '保存蓝图' }).click()
  await expect(page.getByLabel('我的蓝图')).toContainText('MY GATE')
  await expect(page.getByLabel('模拟器状态')).toContainText('蓝图已保存')

  await page.getByLabel('我的蓝图').getByRole('button', { name: /MY GATE/ }).click()
  await expect(page.getByLabel('构造画布')).toContainText('6 元件 · 4 连线')
  ...
  await expect(page.getByLabel('模拟器状态')).toContainText('蓝图已放置 · MY GATE')
...
test('player can encapsulate a primitive circuit as a typed component and reuse it', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '封装组件' }).click()
  await expect(page.getByLabel('我的组件')).toContainText('MY THRESHOLD')
  await expect(page.getByLabel('模拟器状态')).toContainText('组件已封装 · MY THRESHOLD · 1 入 / 1 出')
  ...
  await interfaceEditor.getByRole('button', { name: '完成' }).click()
  ...
  await expect(component).toContainText('黑盒组件')
  await expect(page.getByLabel('构造画布')).toContainText('5 元件 · 3 连线')
  ...
  await expect(page.getByLabel('模拟器状态')).toContainText('组件已封装 · MY DEPLOY GATE · 1 入 / 1 出')
...
test('player can open a black box, edit its primitives, and keep external wiring intact', async ({ page }) => {
  ...
  await expect(page.getByLabel('模拟器状态')).toContainText('进入组件 · OPENABLE THRESHOLD')
  ...
  await scope.getByRole('button', { name: '另存为新组件' }).click()
  ...
  await expect(page.getByLabel('模拟器状态')).toContainText('组件分支已保存 · STRICT THRESHOLD')
  ...
  await page.getByLabel('组件编辑作用域').getByRole('button', { name: '收回黑盒' }).click()
  ...
  await expect(page.getByLabel('模拟器状态')).toContainText('退出组件 · OPENABLE THRESHOLD')
  await page.getByRole('button', { name: '▶ PLAY' }).click()
  await expect(page.getByLabel('boolean_output_1 输出值')).toHaveText('FALSE')
...
test('component library updates are versioned and do not silently rewrite sibling instances', async ({ page }) => {
  ...
  await page.getByLabel(/constant_1_unit.*数值/).fill('0.80')
  const scope = page.getByLabel('组件编辑作用域')
  await scope.getByRole('button', { name: '更新组件库定义' }).click()
  await expect(page.getByLabel('模拟器状态')).toContainText('组件库已更新 · VERSIONED GATE v2')
  await expect(library).toContainText('v2')
  await scope.getByRole('button', { name: '收回黑盒' }).click()
...
test('marquee selection turns a circuit fragment into one movable editing unit', async ({ page }) => {
  ...
  await page.mouse.up()
  await expect(page.getByLabel('蓝图工具')).toContainText('已选择 3 个单元')
...
test('player can freeze behavior as a reusable test suite before refactoring the machine', async ({ page }) => {
  ...
  await bench.getByRole('button', { name: '记录当前行为' }).click()
  await page.getByLabel('number_input_1 数值').fill('0.42')
  await bench.getByLabel('测试名称').fill('LOW SCORE')
  await bench.getByRole('button', { name: '记录当前行为' }).click()
  await expect(bench).toContainText('已保存 2 个样例')
  await expect(bench).toContainText('risk-score → ship')
  await bench.getByRole('button', { name: '运行全部测试' }).click()
  await expect(page.getByLabel('模拟器状态')).toContainText('测试套件 · 2/2 通过 · 无回归')

  await page.getByLabel('constant_1 数值').fill('0.80')
  await bench.getByRole('button', { name: '运行全部测试' }).click()
  await expect(page.getByLabel('模拟器状态')).toContainText('测试套件 · 1/2 通过 · 行为已变化')
  await expect(bench.locator('.sim-test-case.fail')).toContainText('HIGH SCORE')
  await expect(bench.locator('.sim-test-case.fail')).toContainText('FALSE / 预期 TRUE')
  await bench.getByRole('button', { name: '载入输入' }).first().click()
  await expect(page.getByLabel('number_input_1 数值')).toHaveValue('0.72')

  await page.reload()
  await expect(page.getByLabel('模拟器测试台')).toContainText('已保存 2 个样例')
})
